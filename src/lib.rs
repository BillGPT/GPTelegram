    ...
    while let Ok(Some(chunk_result)) = timeout(Duration::from_secs(3), stream.next()).await {
        match chunk_result {
            Ok(chunk) => {
                ...
                for trimmed_str in utf8_str.split("data: ") {
                    if trimmed_str.trim().is_empty() {
                        continue;
                    }
                                        ...
                                        choice.get("delta").and_then(|delta| delta.get("content"))
                                    {
                                        if let Some(content_str) = content.as_str() {
                                            ...
                                            if count % update_interval == 0 {
                                                ...
                                                println!("更新间隔: {}\t平均时间: {:?}", update_interval, duration/(update_interval as u32));
                                                start = Instant::now();
                                            }
                                        }
                                    }
...
pub async fn handle_message(bot: Bot, msg: Message) {
